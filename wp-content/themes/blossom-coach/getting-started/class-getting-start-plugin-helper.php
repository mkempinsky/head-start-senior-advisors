...
/**
 * Class Blossom_Coach_Getting_Start_Page_Plugin_Helper
 *
...
class Blossom_Coach_Getting_Start_Page_Plugin_Helper {
	 ...
	 *
	 * @return Blossom_Coach_Getting_Start_Page_Plugin_Helper;
	 */
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blossom_Coach_Getting_Start_Page_Plugin_Helper ) ) {
			self::$instance = new Blossom_Coach_Getting_Start_Page_Plugin_Helper();
		}
	...
	public function get_button_html( $slug, $settings = array() ) {
		...
		switch ( $state ) {
			case 'install':
				$button .= '<a data-redirect="' . esc_url( $redirect ) . '" data-slug="' . esc_attr( $slug ) . '" class="install-now bc-getting-page-install-plugin button  " href="' . esc_url( $nonce ) . '" data-name="' . esc_attr( $slug ) . '" aria-label="Install ' . esc_attr( $slug ) . '">' . __( 'Install and activate', 'blossom-coach' ) . '</a>';
				break;
	...
	public function enqueue_scripts() {
		...
		if( 'themes.php' != $pagenow )
			return;
		wp_register_script( 'bc-start-page-plugin-install', get_template_directory_uri() . '/getting-started/js/recommended-plugin-install.js', array( 'jquery' ), '', true );

		wp_localize_script(
			'bc-start-page-plugin-install',
			'blossom_coach_start_page',
			array(
				'activating' => esc_html__( 'Activating ', 'blossom-coach' ),
			)
		);

		// wp_enqueue_script( 'plugin-install' );
		wp_enqueue_script( 'updates' );
		wp_enqueue_script( 'bc-start-page-plugin-install' );
	}
}
