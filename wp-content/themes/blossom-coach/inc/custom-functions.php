...
function blossom_coach_setup() {
    ...
    */    
    ...
    add_image_size( 'blossom-coach-blog', 390, 290, true );
    add_image_size( 'blossom-coach-latest', 370, 275, true );
...
function blossom_coach_change_comment_form_default_fields( $fields ){    
    ...
    $fields['author'] = '<p class="comment-form-author"><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'blossom-coach' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'blossom-coach' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['url'] = '<p class="comment-form-url"><input id="url" name="url" placeholder="' . esc_attr__( 'Website', 'blossom-coach' ) . '" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>'; 
...
function blossom_coach_change_comment_form_defaults( $defaults ){    
    $defaults['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment*', 'blossom-coach' ) . '" cols="45" rows="8" aria-required="true"></textarea></p>';
...
if( ! function_exists( 'blossom_coach_change_banner_option' ) ) :
/**
* Function to change banner option from 'static_banner' to 'static_nl_banner' for 
* pro theme upgrade compatibility
* @todo Remove this function after couple of updates.
*/    
function blossom_coach_change_banner_option(){
    if( version_compare( BLOSSOM_COACH_THEME_VERSION, '1.0.4', '==' ) ){
        $banner_option = get_theme_mod( 'ed_banner_section', 'slider_banner' );
        if( $banner_option == 'static_banner' ){
            set_theme_mod( 'ed_banner_section', 'static_nl_banner' );
        }
    }
}
endif;
add_action( 'init', 'blossom_coach_change_banner_option' );

if( ! function_exists( 'blossom_coach_redirect_on_activation' ) ) :
/**
 * Redirect to Getting Started page on theme activation
 *
 */
function blossom_coach_redirect_on_activation() {
    global $pagenow;

    if ( is_admin() && 'themes.php' == $pagenow && isset( $_GET['activated'] ) ) {
        wp_redirect( admin_url( "themes.php?page=blossom-coach-starter" ) );
    }
}
endif;
add_action( 'admin_init', 'blossom_coach_redirect_on_activation' );